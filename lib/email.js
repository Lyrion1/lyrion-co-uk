 ...
 * @param {string} options.from - Sender email (optional, uses default from env or config)
 * @returns {Promise<Object>} Result of email send operation
 */
async function sendEmail({ to, subject, html, from }) {
  // Default sender from environment or fallback
  const sender = from || process.env.EMAIL_FROM || 'oracle@lyrion.co.uk';
