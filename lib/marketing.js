...
function getWeeklySignDetails() {
  try {
    ...
    if (!details) {
      // Fallback to Sagittarius if details not found
      const fallbackSign = 'Sagittarius';
      return {
        sign: fallbackSign,
        ...getSignDetails(fallbackSign)
      };
    }
    return {
      sign: sign,
    ...
    // Return Sagittarius as fallback
    return {
      sign: 'Sagittarius',
      ...getSignDetails('Sagittarius')
    };
  }
}
