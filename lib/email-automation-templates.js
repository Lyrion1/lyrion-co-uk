...
const abandonedCartEmails = [
  {
    ...
    generateContent: (customerName = '', cartItems = [], cartTotal = 0) => {
      const greeting = customerName ? `Dear ${customerName},` : 'Celestial Seeker,';
      const itemsHtml = cartItems.map(item => 
        `<li style="margin-bottom: 10px;">${item.name} - £${Number(item.price).toFixed(2)}</li>`
      ).join('');
  ...
  orderConfirmation: {
    ...
    generateContent: (orderDetails = {}) => {
      ...
      return `
        ...
        ${orderNumber ? `<p style="color: #C4A449;">Order #${orderNumber}</p>` : ''}
  ...
  shippingNotification: {
    ...
    generateContent: (orderDetails = {}) => {
      ...
      return `
        ...
        ${orderNumber ? `<p style="color: #C4A449;">Order #${orderNumber}</p>` : ''}
        ${trackingNumber ? `<p style="color: #5a5856;">Tracking: ${trackingNumber}</p>` : ''}
      `;
    }
  },
      ...
      return `
        ...
        ${intention ? `
          <div style="background: linear-gradient(135deg, rgba(196, 164, 73, 0.1) 0%, rgba(244, 239, 232, 0.3) 100%); padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #C4A449;">
            <p style="color: #151311; font-style: italic;">"${intention}"</p>
          </div>
        ` : ''}
        <p style="color: #5a5856; line-height: 1.8;">
