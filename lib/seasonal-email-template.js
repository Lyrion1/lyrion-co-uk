...
const { createEmailTemplate, escapeHtml } = require('./email');
...
function seasonalUpdateEmail(season, topProducts = []) {
  ...
  if (topProducts && topProducts.length > 0) {
    ...
    topProducts.slice(0, 3).forEach(product => {
      const productUrl = product.url || `https://lyrion.co.uk/product.html?id=${escapeHtml(product.id)}`;
      const productImage = escapeHtml(product.image || product.images?.[0] || '');
      ...
      productsHtml += `
        <div style="margin-bottom: 30px; padding-bottom: 30px; border-bottom: 1px solid #f4efe8;">
          ${productImage ? `<img src="${productImage}" alt="${productName}" style="width: 100%; max-width: 400px; height: auto; border-radius: 8px; margin-bottom: 15px;">` : ''}
          <h4 style="color: #151311; font-size: 18px; margin: 10px 0;">${productName}</h4>
          <p style="color: #5a5856; margin: 10px 0;">${productDescription}</p>
